      ...
      <header className="sticky top-0 bg-white/90 backdrop-blur-md border-b border-slate-200 p-4 z-30">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              onClick={() => {
                // ここに設定画面への遷移を入れる（今は仮でアラート）
                alert("設定画面は実装中... ログアウトしますか？");
                // signOut(); // ログアウトさせたい場合はこれを有効に
              }}
              className="relative h-9 w-9 rounded-full overflow-hidden border-2 border-slate-100 hover:border-blue-500 transition-all active:scale-90"
            >
              {session.user?.image ? (
                <img 
