...
export default function Home() {
    ...
    if (Array.isArray(allPostsData)) {
      ...
      if (allEmails.size > 0) {
        try {
          ...
          if (penNameRes.ok) {
            const { penNameMap: fetchedMap } = await penNameRes.json();
  ...
  const getDisplayName = (authorEmail: string | undefined, authorName: string) => {
    ...
    try {
      ...
      const response = await fetch("/api/comments", {
        ...
        body: JSON.stringify({
          postId: postId,
          text: text,
          author: penName || session?.user?.name || "匿名部員", // ペンネーム優先
  ...
  // 2. 未ログイン（紹介画面）
  return (
    <main className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 max-w-2xl mx-auto border-x border-slate-200 dark:border-slate-700 shadow-sm pb-40">
      {/* ヘッダー */}
        <header className="sticky top-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 p-4 z-30">
          <div className="flex justify-between items-center">
            <h1 className="text-xl font-black text-slate-900 dark:text-slate-100">文芸部ポータル</h1>
            ...
            {session ? (
              ...
              <button 
                ...
                className="text-xs bg-blue-600 dark:bg-blue-700 text-white px-4 py-2 rounded-full font-bold hover:bg-blue-700 dark:hover:bg-blue-600 transition"
              >
                ログイン
              </button>
            )}
      ...
      {/* タブナビゲーション */}
      <div className="sticky top-[73px] bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 z-20">
        <div className="flex">
          <button
            onClick={() => setActiveTab("all")}
            className={`flex-1 py-3 text-sm font-bold transition-all ${
              activeTab === "all"
                ? "text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 bg-blue-50 dark:bg-blue-950"
          ...
          <button
            ...
            className={`flex-1 py-3 text-sm font-bold transition-all ${
              activeTab === "topics"
                ? "text-purple-600 dark:text-purple-400 border-b-2 border-purple-600 bg-purple-50 dark:bg-purple-950"
                : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900"
            }`}
          >
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-slate-900 dark:text-slate-100">{getDisplayName((post as any).authorEmail, post.author)}</span>
                  {post.isTopicPost === 1 ? (
                    ...
                    <span className="text-[10px] bg-green-200 dark:bg-green-900 px-2 py-0.5 rounded text-green-700 dark:text-green-200 font-bold uppercase tracking-wider">返信</span>
                  ) : (
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-1.5 px-2 py-1 rounded-full transition-all duration-300 ${
                      likedPosts.includes(post.id) 
                        ? "text-pink-500 dark:text-pink-400 bg-pink-50 dark:bg-pink-950" 
                        : "text-slate-400 dark:text-slate-600 hover:text-pink-400 dark:hover:text-pink-400 hover:bg-slate-100 dark:hover:bg-slate-800"
                    }`}
                  >
              ...
              <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-1">{post.title}</h2>
              {openPostId === post.id ? (
                <div className="mt-4 p-5 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-inner">
                  <div className="flex justify-between items-start">
                    <p className="whitespace-pre-wrap text-slate-800 dark:text-slate-200 leading-relaxed font-medium text-sm sm:text-base flex-1">
                      {post.body}
                    </p>
                    {session?.user?.name === post.author && (
                      <button
                        ...
                        className="ml-3 text-red-400 dark:text-red-500 hover:text-red-600 dark:hover:text-red-400 transition-colors"
                        title="削除"
                      >
                ...
                <p className="text-slate-400 dark:text-slate-500 text-xs font-medium">クリックして内容を読む...</p>
              )}
            </article>
          ))}
          {allPosts.length === 0 && (
          ...
          <div className="divide-y divide-slate-200 dark:divide-slate-800">
            {topicPosts.map((topic) => (
              <div key={topic.id}>
                {/* お題投稿 */}
                <article 
                  className="p-4 hover:bg-purple-100 dark:hover:bg-purple-900 cursor-pointer transition-colors bg-white dark:bg-slate-950" 
                  onClick={() => router.push(`/topic/${topic.id}`)}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-purple-900 dark:text-purple-200">{getDisplayName((topic as any).authorEmail, topic.author)}</span>
                      <span className="text-[10px] bg-purple-200 dark:bg-purple-900 px-2 py-0.5 rounded text-purple-700 dark:text-purple-200 font-bold uppercase tracking-wider">お題</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <button 
                  ...
                  <h2 className="text-lg font-bold text-purple-900 dark:text-purple-200 mb-1">{topic.title}</h2>
                  <p className="text-slate-600 dark:text-slate-400 text-sm line-clamp-2">{topic.body}</p>
                  <p className="text-slate-400 dark:text-slate-500 text-xs font-medium mt-2">クリックして詳細ページを表示...</p>
                </article>
              </div>
            ))}
          </div>
      ...
      <div className="fixed bottom-8 right-8 z-50 flex flex-col items-end gap-4">
            ...
            {activeTab === "topics" ? (
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full text-sm border border-purple-200 dark:border-purple-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300 dark:focus:ring-purple-600"
                />
                <textarea
                  ...
                  rows={4}
                  className="w-full text-sm border border-purple-200 dark:border-purple-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300 dark:focus:ring-purple-600 resize-none"
                />
                {newPost.title && newPost.body && (
                  <button 
                    ...
                    className="w-full bg-purple-600 dark:bg-purple-700 text-white py-2 rounded-lg text-sm font-bold hover:bg-purple-700 dark:hover:bg-purple-600 shadow-md shadow-purple-200 dark:shadow-purple-900 transition-all"
                  >
                    お題を公開
                  </button>
                )}
              ...
              <>
                <input 
                  ...
                  className="block w-full text-xs text-slate-500 dark:text-slate-400 file:mr-3 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-50 dark:file:bg-blue-900 file:text-blue-700 dark:file:text-blue-200 hover:file:bg-blue-100 dark:hover:file:bg-blue-800" 
                />
                
                {newPost.title && (
                  <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-950 rounded-xl border border-blue-100 dark:border-blue-900">
                    <p className="text-[10px] text-blue-400 dark:text-blue-500 font-bold uppercase mb-1">Ready to Upload</p>
                    <p className="text-sm font-bold text-blue-900 dark:text-blue-200 truncate mb-3">{newPost.title}</p>
                    <button 
                      onClick={async () => {
                        await saveToAWS("regular");
                        setIsFormOpen(false);
                      }} 
                      className="w-full bg-blue-600 dark:bg-blue-700 text-white py-2 rounded-lg text-sm font-bold hover:bg-blue-700 dark:hover:bg-blue-600 shadow-md shadow-blue-200 dark:shadow-blue-900 transition-all"
                    >
                      保存を実行
                    </button>
                  </div>
        ...
        <button
          ...
          className={`${
            session ? "bg-blue-600 dark:bg-blue-700 shadow-blue-200 dark:shadow-blue-900" : "bg-slate-900 dark:bg-slate-800 shadow-slate-200 dark:shadow-slate-800"
